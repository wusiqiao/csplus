...
class WxPayApi {
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6) {
        ...
        $result = WxPayResults::Init($response,$inputObj->GetAPPKey());
     ...
     */
    public static function notify($callback, &$msg,$AppKey = '') {
        //获取通知的数据
        $xml = file_get_contents('php://input'); //$GLOBALS['HTTP_RAW_POST_DATA'];
        //如果返回成功则验证签名
        try {
            $result = WxPayResults::Init($xml,$AppKey);
        } catch (WxPayException $e) {
            $msg = $e->errorMessage();
            \Think\Log::write('notify'.$e->errorMessage());
            return false;
        }
