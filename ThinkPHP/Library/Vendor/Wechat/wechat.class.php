 ...
 * 			'appsecret'=>'xxxxxxxxxxxxxxxxxxx' //填写高级调用功get_all_private_template能的密钥
 * 		);
...
class Wechat {
    ...
    const MENU_CREATE_CONDITONAL_URL = "/menu/addconditional?";
    const MENU_MATCH_URL = '/menu/trymatch?'; //个性化菜单匹配结果
    ...
    //标签管理
    const TAG_CREATE_URL = '/tags/create?'; //创建标签
    const TAG_GET_URL = '/tags/get?'; //获取公众号已创建的标签
    const TAG_UPDATE_URL = '/tags/update?'; //编辑标签
    const TAG_DELETE_URL = '/tags/delete?'; //删除标签
    const TAG_MEMBER_BATCHUPDATE_URL = '/tags/members/batchtagging?'; //批量为用户打标签
    const TAG_MEMBER_BATCHUNTAG_URL = '/tags/members/batchuntagging?'; //批量为用户取消标签
    const TAG_MEMBER_IDLIST_URL = '/tags/getidlist?'; //获取用户身上的标签列表
    ...
    const TEMPLATE_DELETE_TPL_URL = '/template/del_private_template?';
    const TEMPLATE_SEND_URL = '/message/template/send?';
    const TEMPLATE_LIST_URL = '/template/get_all_private_template';
    ...
    const SNS_OAUTH2_URL = '/sns/oauth2/access_token?';
    ///数据分析接口
    ...
    public function __construct($options) {
        ...
        $this->wx_id = isset($options['wx_id']) ? $options['wx_id'] : '';
        isset($options['receive']) ? $this->_receive = $options['receive'] : '';
    }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     *网页授权,通过code换取网页授权access_token和openid，后续需要获取用户全部信息，调用getOauthUserInfo
     * @param type $code
     * @return array(access_token, openid) or false
     */
    public function getOauthUserAccessToken($code){
        return getUserBaseByCode($code);
    }
    //网页授权,通过code换取网页授权access_token和openid
    public function getUserBaseByCode($code){
        $url_get_query = 'appid=' . $this->appid . '&secret=' . $this->appsecret . '&code='.$code . '&grant_type=authorization_code';
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::SNS_OAUTH2_URL . $url_get_query);
        if ($result) {
            ...
            $this->_receive['FromUserName'] = $json['openid'];
            return $json;
        }
        return false;
    }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '') {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getJsTicket($appid, $jsapi_ticket);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
...
// :TODO: Is there any more datatype we should be in the lookout for? (Object?)
    ...
    public function getJsCardTicket($appid = '', $api_ticket = '') {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getJsCardTicket($appid, $api_ticket);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function createMenu($data) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::MENU_CREATE_URL . 'access_token=' . $this->access_token, $json);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
    ...
    public function createConditionalMenu($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $json = self::json_encode($data);
        $result = $this->http_post(self::API_URL_PREFIX . self::MENU_CREATE_CONDITONAL_URL . 'access_token=' . $this->access_token, $json);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->createConditionalMenu($data);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
     ...
     * @return array('menu'=>array(....s))
     */
    public function getMatchMenu($openid) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array("user_id"=>$openid);
        $result = $this->http_post(self::API_URL_PREFIX . self::MENU_MATCH_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || isset($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getMatchMenu($openid);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function uploadMedia($data, $type) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->uploadMedia($data, $type);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
        ...
        if ($result) {
            if (is_string($result)) {
                ...
                if (isset($json['errcode'])) {
                    if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                        $this->resetAuth();
                        return $this->getMedia($media_id, $is_video);
                    }
                    $this->errCode = $json['errcode'];
                    $this->errMsg = $json['errmsg'];
                    return false;
    ...
    public function uploadForeverMedia($data, $type, $is_video = false, $video_info = array()) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->uploadForeverMedia($data, $type, $is_video, $video_info);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function uploadForeverArticles($data) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->uploadForeverArticles($data);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function updateForeverArticles($media_id, $data, $index = 0) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->updateForeverArticles($media_id, $data, $index);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
            ...
            if (is_string($result)) {
                ...
                if ($json) {
                    if (isset($json['errcode'])) {
                        if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                            $this->resetAuth();
                            return $this->getForeverMedia($media_id, $is_video);
                        }
                        $this->errCode = $json['errcode'];
                        $this->errMsg = $json['errmsg'];
                        return false;
    ...
    public function delForeverMedia($media_id) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->delForeverMedia($media_id);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getForeverList($type, $offset, $count) {
        ...
        if ($result) {
            ...
            if (isset($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getForeverList($type, $offset, $count);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function uploadArticles($data) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->uploadArticles($data);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function uploadMpVideo($data) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->uploadMpVideo($data);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function sendGroupMassMessage($data) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->sendGroupMassMessage($data);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function deleteMassMessage($msg_id) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->deleteMassMessage($msg_id);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function queryMassMessage($msg_id) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->queryMassMessage($msg_id);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getDatacube($type, $subtype, $begin_date, $end_date = '') {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getDatacube($type, $subtype, $begin_date, $end_date);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function updateUserRemark($openid, $remark) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->updateUserRemark($openid, $remark);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getUserGroup($openid) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getUserGroup($openid);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function batchUpdateGroupMembers($groupid, $openid_list) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->batchUpdateGroupMembers($groupid, $openid_list);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function updateTag($tagid, $name) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->updateTag($tagid, $name);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function updateMemberTag($tagid, $openid){
        $result = $this->batchUpdateTagMembers($tagid, $openid);
        if (!$result){
            $this->resetAuth();
            $result = $this->batchUpdateTagMembers($tagid, $openid);
        }
    ...
    public function batchUpdateTagMembers($tagid, $openid_list) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->batchUpdateTagMembers($tagid, $openid_list);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
     ...
     * @return boolean|array
     */
    public function batchUnTagMembers($tagid, $openid_list) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
            'openid_list' => $openid_list,
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->batchUnTagMembers($tagid, $openid_list);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getUserTags($openid) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getUserTags($openid);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
            } else
                if (isset($json['tagid_list']))
    ...
    public function getOauthUserinfo($access_token, $openid) {
        if (!self::isRemoteHost() || !self::isWechatBrower()){
            return array("head_pic"=>".", "nickname"=>'PC用户'); //测试专用，在本地才会赋1.
        }
    ...
    public function setTMIndustry($id1, $id2 = '') {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->setTMIndustry($id1, $id2);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
     ...
     * 模板消息 添加消息模板
     * 成功返回消息模板的调用id
     * @param string $tpl_id_short 模板库中模板的编号，有“TM**”和“OPENTMTM**”等形式
     * @return boolean|string
     */
    public function addTemplateMessage($tpl_id_short) {
        $data = array('template_id_short' => $tpl_id_short);
        if (!$this->access_token && !$this->checkAuth())
            return false;
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->addTemplateMessage($tpl_id_short);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    /**删除模板消息
     * @param $tpl_id  公众帐号下模板消息ID
     */
    public function deleteTemplateMessage($tpl_id) {
        $data = array('template_id' => $tpl_id);
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::TEMPLATE_DELETE_TPL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->addTemplateMessage($tpl_id);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getTemplateMessageList() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_URL_PREFIX . self::TEMPLATE_LIST_URL . '?access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getTemplateMessageList();
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function createKFSession($openid, $kf_account, $text = '') {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->createKFSession($openid, $kf_account, $text);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function closeKFSession($openid, $kf_account, $text = '') {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->closeKFSession($openid, $kf_account, $text);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getKFSession($openid) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getKFSession($openid);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getKFSessionlist($kf_account) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getKFSessionlist($kf_account);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function addKFAccount($account, $nickname, $password) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->addKFAccount($account, $nickname, $password);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function updateKFAccount($account, $nickname, $password) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->updateKFAccount($account, $nickname, $password);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function deleteKFAccount($account) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->deleteKFAccount($account);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function setKFHeadImg($account, $imgfile) {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->setKFHeadImg($account, $imgfile);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function querySemantic($uid, $query, $category, $latitude = 0, $longitude = 0, $city = "", $region = "") {
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->querySemantic($uid, $query, $category, $latitude, $longitude, $city, $region);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getOrderByFilter($status = null, $begintime = null, $endtime = null) {
        ...
        if ($result) {
            ...
            if (isset($json['errcode']) && $json['errcode']) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->getOrderByFilter($status, $begintime, $endtime);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function setOrderDelivery($order_id, $need_delivery = 0, $delivery_company = null, $delivery_track_no = null, $is_others = 0) {
        ...
        if ($result) {
            ...
            if (isset($json['errcode']) && $json['errcode']) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->setOrderDelivery($order_id, $need_delivery, $delivery_company, $delivery_track_no, $is_others);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function closeOrder($order_id) {
        ...
        if ($result) {
            ...
            if (isset($json['errcode']) && $json['errcode']) {
                if ($this->isAccessTokenExpire($json['errcode'])){ //是否过期
                    $this->resetAuth();
                    return $this->closeOrder($order_id);
                }
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    /**
     * token是否过期，用5分钟来判断，42001或40014,40001为此错误，
     * 为了防止出现死循环，加入时间过期判断
     */
    private function isAccessTokenExpire($err_code){
        $key = "ERROR_$err_code";
        if ($this->getCache($key)){
            return false;
        }
        $this->setCache($key, strval(time()), 1);
        if ($err_code == 40001 || $err_code == 42001 || $err_code == 40014){
            return true;
        }
        return false;
    }
    ...
    /**小程序二维码 B接口
     * @param $scene 场景
     * @param $path  路径:"pages/index/index"
     * @param $width  宽度
     * @return array|bool|mixed|string
     */
    public function getMPQrcodeB($scene, $path, $width = 135) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $json = self::json_encode(array("scene"=>$scene, "path"=>$path, "width"=>$width));
        $result = $this->http_post(self::API_BASE_URL_PREFIX . '/wxa/getwxacodeunlimit?access_token=' . $this->access_token, $json);
        return $result;
    }

    /**小程序二维码C接口
     * @param $path  路径+参数:"pages/index?query=1"
     * @param $width  宽度
     * @return array|bool|mixed|string
     */
    public function getMPQrcodeC($path, $width = 135) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $json = self::json_encode(array("path"=>$path, "width"=>$width));
        $result = $this->http_post(self::API_URL_PREFIX . '/wxaapp/createwxaqrcode?access_token=' . $this->access_token, $json);
        return $result;
    }
