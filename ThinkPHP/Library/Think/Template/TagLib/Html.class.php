...
class Html extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'select'    => array('attr'=>'name,options,values,output,multiple,id,size,first,change,selected,dblclick,placeholder','close'=>0),
	...
	$onchange   = $tag['change'];
        ...
        if(!empty($multiple)) {
            $parseStr = '<select id="'.$id.'" name="'.$name.'" ondblclick="'.$ondblclick.'" onchange="'.$onchange.'" multiple="multiple" class="'.$style.'" size="'.$size.'" data-placeholder="'.$placeholder.'" >';
        }else {
            $parseStr = '<select id="'.$id.'" name="'.$name.'" onchange="'.$onchange.'" ondblclick="'.$ondblclick.'" class="'.$style.'" data-placeholder="'.$placeholder.'" >';
        }
        if(!empty($first)) {
