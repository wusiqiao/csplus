...
//        // 模型初始化  --------->移动到函数后面比较合适akunzeng 20151028
//        $this->_initialize();
        // 获取模型名称
        if(!empty($name)) {
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        if ($options['callback']){ //akunzeng 2016/2/4默认不触发回调
    ...
    //akunzeng 2016/2/4 新增函数
    public function update($data = null){
        $saveOoptions["callback"] = true; //触发回调，默认不触发
        return $this->save($data, $saveOoptions);
    }
    ...
    protected function _parseOptions($options=array()) {
        ...
        // 表达式过滤 ,默认是必须过滤 akunzeng 2015/06/22
        if (isset($options['call_filter'])){
           $call_filter =  $options['call_filter'];
        }else{
           $call_filter = true;     
        }
        if ($call_filter){
    ...
    /*akunzeng 2015/06/22*/
     public function callFilter($call){
        $this->options['call_filter'] = $call;
        return $this;
    }
    
    protected function parseWhere($where){
       return $this->db->parseWhere($where);
    }
