...
function mergeDefaultOptions(options) {
  var defaults = {
    ...
    searchable: false,
    searchParams: ['value', 'text', 'description', 'section','cityid'],
    sectionDelimiter: '/',
...
"use strict";
//城市改动
module.exports = function (id, value, text, description,initialIndex,section,cityid,cityname) {
  this.id = id;
  this.value = value;
  this.text = text;
...
Tree.prototype.generateSelections = function () {
  ...
  this.$originalSelect.find('> option').each(function () {
    ...
    //城市改动
    var cityid = option.getAttribute('data-cityid');
    var cityname = option.getAttribute('data-cityname');
    ...
    //城市改动
    var optionObj = new Option(id, optionValue, optionName, optionDescription, optionIndex, section,cityid,cityname);
    if (optionIndex) {
...
function createSelected(option, disableRemoval, showSectionOnSelected) {
  var node = createNode('div', {
    ...
    text: option.text,
    //城市改动
    'data-cityid':option.cityid,
    'data-name':option.cityname,
  });
