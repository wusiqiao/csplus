 ...
 * @option_param    {string}   rota          顺时针旋转上传图片按钮的选择器或者DOM对象
	...
	function photoClip(container, option) {
		...
		if ($rota.length) {
			$rota.on('click', function() {
				rotateCW({	x: ($('#clipArea').width() + 40)/2,
							y: ($('#clipArea').height() + 40)/2}
						)
			});
		}
		var $win = $(window);
...
//originWidt|horiginHeight
			...
			myScroll.scrollTo(posX, posY);//原图片显示位置
		}
