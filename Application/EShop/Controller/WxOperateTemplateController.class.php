...
class WxOperateTemplateController extends BaseController{
    ...
    public function indexAction(){
		...
		$list = D('SysUserModuleSetting')->alias('s')
			...
			->where(array('s.module'=>'WxOperateTemplate','s.type'=>DAC_SETTING_TYPE_BRANCH,'s.branch_id'=>$this->companyId,'s.user_id'=>$this->userId))
			->group('b.id')
			->select();
    ...
    public function listAction(){
		...
		$lists = M('WxNoticeTemplateLibrary')->alias('wntl')
			->field('b.id as company_id,b.name as company_name,FROM_UNIXTIME(wntl.life,\'%m月\') as month,wntl.life,wnru.created_at,wnru.message_state,count(wnru.id) as number')
			->join('LEFT JOIN wx_notice_relation_user as wnru ON wnru.notice_id = wntl.id')
