...
//                    }
//                    $this->add_timer($delay, "/admin.php/ReqQueue/send_wx_message/id/$result");
//                } else {
...
//    }
     ...
     * @return type
     */
    private function sendwx_internal($data) {
        ...
        if (!$wechat->sendTemplateMessage($message)) {
            ...
            \Think\Log::write("send_wx_message error!template_id:" . $message["template_id"] . "message:" . $wechat->errMsg);
        }
        return $result;
    }
    //回调
        ...
        if ($id) {
            ...
            if ($msg) {
                ...
                foreach($package['users'] as $key=>$value){
                    ...
                    $result = $this->sendwx_internal($package['message']);
                    if ($result["errcode"] == 0) {
                ...
                D('WxBranchTemplate')->userSendTemplateFinally($finally);
            }
        }
    }
    //回调
