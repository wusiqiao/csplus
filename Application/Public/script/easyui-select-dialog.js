(function($) {       
    $.fn.easyui_select_dialog = function (options, param) {
        if (typeof options === 'string') {
            var method = $.fn.easyui_select_dialog.methods[options];
            if (method) {
        ...
        return this.each(function () {
            var state = $.data(this, 'easyui_select_dialog');
            if (state) {
                $.extend(state.options, options);
            } else {
                $.data(this, 'easyui_select_dialog', $.extend({}, $.fn.easyui_select_dialog.defaults, options));
                initial(this);
            }
        });
    };
    ...
    function initial(target){
        var state = $.data(target, "easyui_select_dialog");
        var $contentWrap = $("<div class='inputbox'></div>").appendTo($(target)).hide();
    ...
    $.fn.easyui_select_dialog.defaults = $.extend({
        width: "100%",
        height:"auto",
        content:"",
        direction:"bottom"
    });
    $.fn.easyui_select_dialog.methods = {
        reset: function (jq) {
            return jq.each(function () {
              $(this).remove(".content-wrap");
              $(this).removeData("easyui_select_dialog");
            });
        }
    };
