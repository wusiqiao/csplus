...
const FLAG_BILL_RECEIPT = 6; //银行回单
const FLAG_BILL_TAX = 7; //发票
...
const SUBJECT_CATEGORIES = [
    SUBJECT_CATEGORY_CAPITAL,
    SUBJECT_CATEGORY_DEPT,
    SUBJECT_CATEGORY_RIGHTS,
    SUBJECT_CATEGORY_COST,
    SUBJECT_CATEGORY_INCOME
];

const SUBJECT_CATEGORY_CAPITAL = 1;//资产
const SUBJECT_CATEGORY_DEPT = 2;//负债
const SUBJECT_CATEGORY_RIGHTS = 3;//权益
const SUBJECT_CATEGORY_COST = 4;//成本
const SUBJECT_CATEGORY_INCOME = 5;//损益

const ENTERPRISE_SCALE_SMALL = 0; //小规模
const ENTERPRISE_SCALE_GENERAL = 1; //一般纳税人

const VCR_DRAF_REVIEW_ACCOUNTING = 0; //会计审核凭证
const VCR_DRAF_REVIEW_SUP = 1; //主管+会计审核凭证
...
const SUBJECT_IMPORT_COLUMNS = array(
    ...
    "subject_direction_title"=>"借贷方向;余额方向"
);

const BANK_IMPORT_COLUMNS = array(
    "bc_debit"=>"借方;支取;支出;借方金额;转出金额;转入金额",
    "bc_credit"=>"贷方;收入;贷方金额;借贷标志",
    "bc_side"=>"对方户名;对方账号名称;对方单位",
    "bc_summary"=>"摘要",
    "bc_memo"=>"备注;用途",
...
function getSubjectByName($branch_id, $subject_name, $parent_id = null){
    ...
    if (empty($result)){ //找不到再找映射科目
        ...
        if($ent_type_id == null){
            $condition_b['a.ent_type_id'] = 0;
        }elseif(M("VcrSysSubject")->where("ent_type_id=$ent_type_id")->count() == 0){
            $condition_b['a.ent_type_id'] = 0;
        }
...
function getVoucherSubjectById($branch_id,$id){
    return M("VcrSubject")->where("id = $id and branch_id = $branch_id")->getField("name");
}