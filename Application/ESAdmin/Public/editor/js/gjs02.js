...
UE.I18N['zh-cn'] = {
	...
	'insertimage': {
		'static': {
			...
			'lang_tab_upload': "上传图片",
			'lang_tab_online': "公众平台图片管理",
			'lang_tab_search': "在线素材",
			'lang_tab_wxsearch': "万象图片",
			'lang_input_url': "地 址：",
			'lang_input_size': "大 小：",
			'lang_input_width': "宽度",
...
jQuery(function($) {
	$.jslanguage = {
		...
		cancel_fav_confirm: '您确认要删除这条收藏的数据吗？',
		needlogin: '登录',
...
function getEditorHtml() {
	...
	html = html.replace(/localhost/g, 'pic.fzn.cc');
	html = html.replace(/127\.0\.0\.1/g, 'pic.fzn.cc');
	html = html.replace(/border: 1px dotted rgb\(99, 144, 144\)/g, '');
	html = html.replace(/alt\=\"\"/g, 'alt="http://fzn.cc"');
	html = html.replace(/title\=\"\"/g, 'title="http://fzn.cc"');
	html = html.replace(/title\=\"undefined\"/g, 'title="http://fzn.cc"');
	html = html.replace(/alt\=\"undefined\"/g, 'alt="http://fzn.cc"');
	html = html.replace(/http:\/\/weixinhow.com\/images/g, 'https://mmbiz.qlogo.cn');
	html = html.replace(/\?wx_fmt\=gif/g, '.gif');
	html = html.replace(/\/mmbizgif/g, '.gif');
	html = html.replace(/src\=\"\//g, 'src="http://pic.fzn.cc/');;
	html = html.replace(/http\:\/\/editor\.fzn\.cc\/img\.php\?url\=/g, '');
	...
	htmlObj.find('.weixinhow.com').each(function() {
	...
	if (tizi_brush_on && !durring_pcc) {
		...
		if (select_html != "") {
			...
			select_obj.find('h1,h2,h3,h4,h5,h6').each(function(i) {
				var title = obj.find('.v3title').eq(i);
			...
			var bgimg_size = obj.find('.v3bg').size();
			select_obj.find('img').each(function(i) {
				var bgimg = obj.find('.v3bg').eq(i);
			...
			var brushs = obj.find('.v3brush');
			var total = brushs.size();
			if (total > 0) {
				if (total == 1) {
					var brush_item = obj.find('.v3brush:first');
...
function setBackgroundColor(bgcolor, color, history) {
	...
	if (!$('#replace-color-all')[0].checked && current_active_v3item) {
		parseObject(current_active_v3item, bgcolor, color);
		current_editor.undoManger.save();
	} else {
	...
	obj.find("*").each(function() {
		...
		if ($(this).data('bcless') || hasSetBgColor) {
			...
			if (this.style.borderBottomColor || this.style.borderTopColor || this.style.borderLeftColor || this.style.borderRightColor) {
				...
				setColor(this, 'borderColor', bc_color);
			}
		} else {
			if (this.style.borderBottomColor || this.style.borderTopColor || this.style.borderLeftColor || this.style.borderRightColor) {
...
function saveWxMsg(form) {
	...
	postdata[postdata.length] = {
		name: 'data[WxMsg][content]',
		value: getEditorHtml()
	};
	ajaxAction(form.action, postdata, form, function() {
...
function saveTemplate(form) {
	if (getEditorHtml() == "") {
		showErrorMessage("请先输入模板内容");
		ckeditors['editor'].focus();
		return false;
	}
	...
	publishController.open_dialog('/wx_msgs/save/' + current_edit_msg_id + '?inajax=1&rethtml=1', {
		title: '保存图文',
		'width': 900,
...
function set_style(styles) {
	ckeditors['editor'].execCommand('removeFormat');
	var element = ckeditors['editor'].getSelection().getStartElement();
	if (element.getName() == 'img') {
		...
		ckeditors['editor'].applyStyle(style);
	}
};
...
function applyParagraph(type) {
	...
	if (type == 'all') {
		...
		if (!current_active_v3item) return;
		object = current_active_v3item;
	}
	$(object).find('p').each(function() {
		$(this).css('line-height', $('#paragraph-lineHeight').val());
		$(this).css('font-family', $('#paragraph-fontFamily').val());
		$(this).css('font-size', $('#paragraph-fontSize').val());
		$(this).css('text-indent', $('#paragraph-textIndent').val());
		$(this).css('padding-Top', $('#paragraph-paddingTop').val());
		$(this).css('padding-Bottom', $('#paragraph-paddingBottom').val());
	})
}!
function(a) {
	var b = function() {
...
var publishController = {
	...
	open_dialog: function(url, options, postdata) {
		...
		$dialog.loading_html = '<img src="' + BASEURL + '/img/ajax/wheel_throbber.gif"> 正在加载...', $dialog.dialogid = url.replace(/\/|\.|:|,| |\?|=|&/g, '_') + '-ajax—action';
		...
		if ($('#' + $dialog.dialogid).size() < 1) {
			$('<div  class="modal fade" id="' + $dialog.dialogid + '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h3 id="myModalLabel">' + options.title + '</h3></div><div class="modal-body">' + $dialog.loading_html + '</div></div></div></div>').appendTo('body');
		}
		if (postdata != null) {
			$.ajax({
				type: 'post',
		...
		$('#' + $dialog.dialogid).on('hidden.bs.modal', function(e) {
...
$(function() {
	...
	$('.delete-pop').on('shown.bs.popover', function() {
		var $this = $(this);
...
UE.plugins["sectionv3"] = function() {
	...
	me.addListener("beforepaste", function(name, html, root) {
		var parser = $('<div>' + html.html + '</div>');
		percent_width(parser);
		parser.find('*').each(function() {
			percent_width($(this));
		});
		parser.find('img').each(function() {
			var url = this.src;
			url = url.replace(/http:\/\/mmbiz.qpic.cn/g, 'http://mmbiz.qpic.cn');
	...
	function percent_width(obj) {
		var total_width = 0;
		var nowidth_item = false;
		obj.children().each(function() {
			if ($(this).data('width') && $(this).data('width') != 'fix') {
				this.style.width = $(this).data('width');
				nowidth_item = true;
				return;
			} else if (this.style.width == "" || this.style.width.search('%') > 0 || $(this).data('width') == 'fix' || this.style.width.search('em') > 0) {
				nowidth_item = true;
				return;
			}
			total_width += parseInt(this.style.width.replace('px', ''));
		});
		if (nowidth_item == false && total_width > 120) {
			obj.children().each(function() {
				this.style.width = (parseInt(this.style.width.replace('px', '')) * 100) / total_width + '%';
			});
		}
	}
	var _v3BgDialog = new UE.ui.Dialog({
		iframeUrl: (editor.options.UEDITOR_HOME_URL + 'dialogs/fzneditor/fzneditorbg.html'),
		editor: me,
		name: 'v3BgDialog',
		title: "背景图设置",
		...
		buttons: [{
			...
			label: '确定',
			onclick: function() {
				_v3BgDialog.close(true);
			}
		}, {
			className: 'edui-cancelbutton',
			label: '取消',
			onclick: function() {
				_v3BgDialog.close(false);
			}
		}]
	});
	var _v3BdBgDialog = new UE.ui.Dialog({
		iframeUrl: (editor.options.UEDITOR_HOME_URL + 'dialogs/fzneditor/fzneditor.html'),
		editor: me,
		name: 'v3BdBgDialog',
		title: "边框底纹设置",
		...
		buttons: [{
			...
			label: '确定',
			onclick: function() {
				_v3BdBgDialog.close(true);
			}
		}, {
			className: 'edui-cancelbutton',
			label: '取消',
			onclick: function() {
				_v3BdBgDialog.close(false);
			}
		}]
	});
	var clickPop = new baidu.editor.ui.Popup({
		...
		_v3bg: function() {
			_v3BgDialog.render();
			_v3BgDialog.open();
			_v3BgDialog.anchorEl = clickPop.anchorEl;
		},
		_usecolor: function() {
			var id = $(clickPop.anchorEl).data('id');
			var editor_document = current_editor.selection.document;
			var bgcolor = $(clickPop.anchorEl).attr('data-color');
			if (bgcolor) {
				var num = 0;
				jQuery('.fzneditor[data-id=' + id + ']', editor_document).each(function() {
					if (bgcolor != $(this).attr('data-color')) {
						parseObject($(this), bgcolor, '#FFF');
						$(this).attr('data-color', bgcolor);
						$(this).attr('data-custom', bgcolor);
						num++;
					}
				});
				current_editor.undoManger.save();
		...
		_v3bdbg: function() {
			_v3BdBgDialog.render();
			_v3BdBgDialog.open();
			_v3BdBgDialog.anchorEl = clickPop.anchorEl;
		},
		_v3para: function() {
			publishController.open_html_dialog('paragraph-setting');
		},
		_v3video: function() {
			current_editor.ui._dialogs['insertvideoDialog'] && current_editor.ui._dialogs['insertvideoDialog'].open();
			current_editor.ui._dialogs['insertvideoDialog'].anchorEl = clickPop.anchorEl;
		},
		_v3item: null,
	...
	me.addListener('ready', function(editor) {
		...
		$(editor_document).keydown(function(event) {
			var ctrlKey = 17,
				vKey = 86,
				cKey = 67;
			if (event.ctrlKey && event.keyCode == 67) {
				var html = getDealingHtml(); {
					var html = getSelectionHtml();
					if ($('<div>' + html + '</div>').find('.fzneditor').size() > 0) {
						event.preventDefault();
						event.stopPropagation();
						showErrorMessage("<strong>复制错误！</strong><ul style=\"margin-left: 10px;\"><li style=\"list-style-type: disc;text-align: left;\"><strong>全文复制：</strong>请点击右侧“<button class=\"btn btn-danger btn-sm btn-inline\"><i class=\"fa fa-copy\"></i>复制</button>”按钮复制</li><li style=\"list-style-type: disc;text-align: left;\"><strong>单样式复制：</strong>点击样式选中，通过底部出现的悬浮工具条中的“复制”按钮复制</li><li style=\"list-style-type: disc;text-align: left;\"><strong>文本复制：</strong>仅选中文字（没选择样式）时，支持快捷键“Ctrl+C”复制</li></ul>");
						return false;
					}
				}
			}
		});
		jQuery(editor_document).on('dblclick', 'img', function() {
			current_editor.ui._dialogs['insertimageDialog'] && current_editor.ui._dialogs['insertimageDialog'].open();
		});
	});
        me.addListener('paste', function(t, evt){
           alert('past');
           return false;
        });
	me.addListener("click", function(t, evt) {
		...
		if ($(el).parents('.fzneditor').size() > 0) {
			el = $(el).parents('.fzneditor:first').get(0);
			...
			current_active_v3item = $(el);
			current_active_v3item.css({
				'border': '1px dotted rgb(99, 144, 144)',
				'padding': '2px'
			});
			clickPop.render();
                        //var html = clickPop.formatHtml('<nobr class="otf-poptools">' + '<span class="copy" stateful>' + '复制</span>' + '<span onclick="$$._remove()" stateful>' + '删除</span>' + '<span class="_v3bg hidden" onclick="$$._v3bg()" stateful>' + '背景图</span>' + '<span class="_v3video hidden" onclick="$$._v3video()" stateful>' + '视频</span>' + '<span class="_v3bdbg hidden" onclick="$$._v3bdbg()" stateful>' + '边框</span>' + '<span class="_v3para" onclick="$$._v3para()" stateful>' + '段落</span>' + '<span onclick="$$._blank()" stateful>' + '下空行</span>' + '<span onclick="$$._preblank()" stateful>' + '上空行</span>' + '<br><p style="color:#DB512D;font-size:6px;margin-top:6px;"><strong>&nbsp;提示：</strong>点击右侧配色图中的颜色可更换配色!</p>' + '</nobr>');	
                        var html = clickPop.formatHtml('<nobr class="otf-poptools">' + 
                        '<span class="space-control" onclick="$$._remove()" stateful>' +  '<i class="fa fa-trash-o"></i><span>删除</span></span>' + 
                        '<span class="_v3bg hidden space-control" onclick="$$._v3bg()" stateful>' + '<i class="fa fa-photo"></i><span>背景图</span></span></span>' + 
                        '<span class="_v3video hidden space-control" onclick="$$._v3video()" stateful>' + '<i class="fa fa-video-camera"></i><span>视频</span></span></span>' + 
                        '<span class="_v3para space-control" onclick="$$._v3para()" stateful>' + '<i class="fa fa-paragraph"></i><span>段落</span></span></span>' + 
                        '<span id="flat-add-radius" class="space-control" stateful>' + '<i class="fa fa-circle-o"></i><span>加圆角</span></span></span>' +
                        '<span id="flat-strip-radius" class="space-control" stateful>' + '<i class="fa fa-circle"></i><span>去圆角</span></span></span>' +
                        '<span id="flat-add-border" class="space-control" stateful>' + '<i class="fa fa-square-o"></i><span>加边框</span></span></span>' + 
                        '<span id="flat-strip-border" class="space-control" stateful>' + '<i class="fa fa-square"></i><span>去边框</span></span></span><br>' + 
                        '<span id="flat-add-shadow" class="space-control" stateful>' + '<i class="fa fa-adjust"></i><span>加阴影</span></span></span>' + 
                        '<span id="flat-strip-shadow" class="space-control" stateful>' + '<i class="fa fa-adjust"></i><span>去阴影</span></span></span>' + 
                        '<span id="set-image-radius" class="space-control" stateful>' + '<i class="fa fa-photo"></i><span>图片圆形</span></span></span>' + 
                        '<span id="set-image-border" class="space-control" stateful>' + '<i class="fa fa-photo"></i><span>图片边框</span></span></span>' + 
                        '<span id="v3-random-color" class="space-control" stateful>' + '<i class="fa fa-random"></i><span>随机换色</span></span></span><br>' + 
                        '<p style="color:#DB512D;font-size:6px;margin-top:6px;"><strong>&nbsp;提示：</strong>点击右侧配色图中的颜色可更换配色!</p>' + '</nobr>');
			var content = clickPop.getDom('content');
			content.innerHTML = html;
			$(el).find('.select').tooltip({
				'title': '选中后，可以秒刷更换为别的样式'
			});
			if ($(el).find('.v3bg').size()) {
				$(content).find('._v3bg').removeClass('hidden');
			}
			if ($(el).find('.video_iframe').size()) {
				$(content).find('._v3video').removeClass('hidden');
			}
			var bdbg_flag = false;
			...
			if (bdbg_flag && $.browser.webkit) {
				$(content).find('._v3bdbg').removeClass('hidden');
			}
			clickPop.anchorEl = el;
			clickPop.showAnchor(clickPop.anchorEl);
//                        var clipboard = new ClipboardJS('.copy', {
//                            text: function() {
//                                return "<p>" + $(clickPop.anchorEl).prop('outerHTML') + "</p>";
//                            }
//                        });
...
//                        });
			...
			if (current_active_v3item) {
				//current_active_v3item.removeAttr('style');
                                var bstyle = current_active_v3item.css("border-style");
                                if (bstyle == "dotted"){
                                    current_active_v3item.css("border","none");
                                }
...
function(a, b) {
	a.MixItUp = function() {
		...
		_refresh: function(a) {
			var c = this;
			c._execAction("_refresh", 0, arguments), c._$targets = c._$container.find(c.selectors.target);
		...
		_bindHandlers: function() {
			var b = this;
			b._execAction("_bindHandlers", 0), b.controls.live ? b._$body.on("click.mixItUp." + b._id, b.selectors.sort, function() {
				b._processClick(a(this), "sort")
			}).on("click.mixItUp." + b._id, b.selectors.filter, function() {
				b._processClick(a(this), "filter")
			}) : (b._$sortButtons = a(b.selectors.sort), b._$filterButtons = a(b.selectors.filter), b._$sortButtons.on("click.mixItUp." + b._id, function() {
				b._processClick(a(this), "sort")
		...
		_processClick: function(b, c) {
			...
			if (d._execAction("_processClick", 0, arguments), !d._mixing || d.animation.queue && d._queue.length < d.animation.queueLimit) {
				if (d._clicking = !0, "sort" === c) {
					...
					(!b.hasClass(d.controls.activeClass) || e.indexOf("random") > -1) && (a(d.selectors.sort).removeClass(d.controls.activeClass), b.addClass(d.controls.activeClass), d.sort(e))
				}
				if ("filter" === c) {
					var f, g = b.attr("data-filter"),
						h = "or" === d.controls.toggleLogic ? "," : "";
					d.controls.toggleFilterButtons ? (d._buildToggleArray(), b.hasClass(d.controls.activeClass) ? (b.removeClass(d.controls.activeClass), f = d._toggleArray.indexOf(g), d._toggleArray.splice(f, 1)) : (b.addClass(d.controls.activeClass), d._toggleArray.push(g)), d._toggleArray = a.grep(d._toggleArray, function(a) {
						return a
			...
			} else "function" == typeof d.callbacks.onMixBusy && d.callbacks.onMixBusy.call(d._domNode, d._state, d), d._execAction("_processClickBusy", 1, arguments)
		},
		...
		_buildState: function(a) {
			...
			}, b._execAction("_buildState", 1), a ? b._execFilter("_buildState", c) : (b._state = c, void 0)
		},
		_goMix: function(a) {
			var b = this,
				c = function() {
				...
				d = function() {
					...
					b._getInterMixData(), b._setFinal(), b._getFinalMixData(), a + d > window.innerHeight && window.scrollTo(c, a), b._prepTargets(), window.requestAnimationFrame ? requestAnimationFrame(e) : setTimeout(function() {
						e()
					}, 20)
				},
				e = function() {
		...
		insert: function() {
			...
			a._execAction("insert", 1, arguments), "object" == typeof b.multiMix ? a.multiMix(b.multiMix, c) : a._refresh()
		},
		prepend: function() {
			var a = this,
				b = a._parseInsertArgs(arguments);
