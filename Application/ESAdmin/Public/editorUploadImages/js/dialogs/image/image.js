...
(function () {
    ...
    RemoteImage.prototype = {
        ...
        setImage: function(img){
            ...
            if(src) {
                /* 设置表单内容 */
                var percent = img.height / img.width * 100;
                $G("width").value = 100 || '';
                $G("height").value = percent.toFixed(0) || '';
            ...
            uploader.on('all', function (type, files) {
                switch (type) {
                    ...
                    case 'startUpload':
                        ...
                        //新增url拼接model(上传文件文件名) lynn 0403
                        if(editor.getOpt('upload_model') != ''){
                            url = url + '&model='+editor.getOpt('upload_model');
                        }
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                if (li.tagName.toLowerCase() == 'li') {
                    if (domUtils.hasClass(li, 'selected')) {
                        ...
                        //修改 图片单选 lynn 0403
                        var liAll = li.parentNode.childNodes;
                        if(liAll.length > 0){
                            for(i = 0;i< liAll.length;i++){
                                domUtils.removeClasses(li.parentNode.children[i],'selected');
                            }
                        }
